...
import { userLoadActionSuccess, userPostsLoadActionSuccess } from '../actions/user';
...
function* userLoad(
  params: UserAction,
): Generator<AllEffect<CallEffect<any>> | PutEffect<UserAction>, void, [any, any]> {
  try {
    switch (params.type) {
      case USER_LOAD: {
    ...
    switch (params.type) {
      case USER_POSTS_LOAD: {
        ...
        const [posts] = yield all([
          call(getPostsByUser, params.id as string, params.page as number, params.limit as number),
        ]);
